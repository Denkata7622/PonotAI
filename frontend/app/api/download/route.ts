...
function parseYoutubeUrl(input?: string): string | undefined {
  ...
  try {
    ...
    if (isYoutubeHost && url.pathname === "/watch") {
      ...
      return YOUTUBE_ID_REGEX.test(id) && !isFakeYoutubeId(id) ? `https://www.youtube.com/watch?v=${id}` : undefined;
    }
    if (host === "youtu.be") {
...
function jsonError(error: string, code: string, fix: string, status: number): NextResponse {
  return NextResponse.json({ error, code, fix }, { status });
}

function resolveDownloadTarget(body: DownloadBody): { target?: DownloadTarget; response?: NextResponse } {
    ...
    if (!parsedUrl) {
      return {
        response: jsonError(
          "Invalid YouTube URL",
          "invalid-youtube-url",
          "Provide a real YouTube watch URL or youtu.be URL from youtube.com, a YouTube video ID, or a search query.",
          400,
        ),
      };
    }
    return { target: { target: parsedUrl, type: "url" } };
  }
  ...
  if (parsedPlatformUrl) return { target: { target: parsedPlatformUrl, type: "url" } };
...
function downloadHeaders(audioLength: number, filename: string, cache: "hit" | "miss", targetType: DownloadTarget["type"]): HeadersInit {
  return {
...
export async function handleDownloadPost(request: Request, runner: YtdlpRunner = defaultRunner): Promise<Response> {
  ...
  try {
    ...
    return jsonError("Invalid JSON body", "invalid-json", "Send valid JSON with youtubeId or query.", 400);
  }

  const resolved = resolveDownloadTarget(body || {});
  ...
  try {
    ...
    if (cached) {
      return new Response(new Uint8Array(cached.audio), {
        status: 200,
        headers: downloadHeaders(cached.audio.byteLength, cached.filename, "hit", target.type),
      });
    }
    ...
    if (result.code !== 0) {
      const fullDetail = `${result.stderr || ""}\n${result.stdout || ""}`.trim() || `yt-dlp exited with code ${result.code}`;
      const classified = classifyDownloadError(fullDetail);
      return NextResponse.json({ error: "YouTube download failed.", code: classified.code, detail: classified.detail, fix: classified.fix }, { status: classified.status });
    }
    ...
    return new Response(new Uint8Array(audio), {
      status: 200,
      headers: downloadHeaders(audio.byteLength, mp3, "miss", target.type),
    });
  } catch (error) {
