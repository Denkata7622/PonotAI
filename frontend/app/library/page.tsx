  ...
  <div className="min-h-96">
      ...
      <section className="card p-6 space-y-4">
        ...
        <div className="space-y-2">
          {filteredHistory.length === 0 ? (
            <p className="cardText">{t("history_empty", language)}</p>
          ) : (
            filteredHistory.map((item) => (
              ...
              >
                ...
                <button onClick={() => handlePlaySong(item)} className="rounded-full bg-[var(--accent)] p-2.5 text-white opacity-0 transition group-hover:opacity-100" title="Play">▶</button>
              </div>
            ))
          )}
        </div>
          ...
          {filteredFavorites.length > 0 ? (
            filteredFavorites.map((fav, idx) => (
              <div key={fav.id ?? idx} className="group flex items-center justify-between gap-4 rounded-xl border border-[var(--border)] bg-[var(--surface)] px-4 py-3 transition hover:border-[var(--accent)]/50 hover:bg-[var(--surface-2)]">
                ...
                <button onClick={() => handlePlaySong(fav)} className="rounded-full bg-[var(--accent)] p-2.5 text-white opacity-0 transition group-hover:opacity-100" title="Play">▶</button>
              </div>
            ))
          ) : (
    ...
    {selectedTab === "playlists" && (
      <div className="space-y-4">
        {isAuthenticated && (
          <div className="card p-4">
            {!showNewPlaylistInput ? (
              <Button onClick={() => setShowNewPlaylistInput(true)} className="w-full">+ Create New Playlist</Button>
            ) : (
        ...
        {loading ? (
          <div className="card p-12 text-center"><p className="cardText">Loading playlists...</p></div>
        ) : filteredPlaylists.length === 0 ? (
          <div className="col-span-full card p-12 text-center"><p className="cardText">{searchQuery ? "No playlists match your search" : "No playlists created yet."}</p></div>
        ) : (
