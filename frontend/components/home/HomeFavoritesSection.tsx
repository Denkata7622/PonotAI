...
export default function HomeFavoritesSection({
  ...
  if (favoritesSet.size === 0) return null;
          ...
          return (
            <div key={trackId} className="group relative flex items-center gap-4 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-3 transition-all hover:border-[var(--accent)]/50 hover:shadow-lg">
              ...
              <div className="relative">
                <button aria-label="Favorite item options" onClick={() => setFavoritesMenuOpen(isMenuOpen ? null : trackId)} className="rounded-lg p-2 opacity-0 transition group-hover:opacity-100 hover:bg-surface-raised">⋯</button>
                {isMenuOpen && (
